...
pub fn auth_routes() -> Scope {
    web::scope("/auth")
        ...
        .route(
            ...
            web::post().to(resend_email_verification_handler),
        )
}

async fn register_handler(req: HttpRequest, data: web::Json<RegisterUserRequest>) -> HttpResponse {
    let auth_controller = req
        .app_data::<web::Data<AuthController>>()
...
async fn login_handler(req: HttpRequest, data: web::Json<LoginRequest>) -> HttpResponse {
    let auth_controller = req
        .app_data::<web::Data<AuthController>>()
        .expect("AuthController not found in app data");
...
async fn logout_handler(req: HttpRequest, data: web::Json<LogoutRequest>) -> HttpResponse {
    let auth_controller = req
        .app_data::<web::Data<AuthController>>()
        .expect("AuthController not found in app data");
...
async fn resend_email_verification_handler(
    ...
    auth_controller
        .resend_email_verification(data)
        .await
        .unwrap_or_else(|e| {
            HttpResponse::InternalServerError().json(serde_json::json!({
                "error": e.to_string()
