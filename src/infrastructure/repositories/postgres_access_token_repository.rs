...
use crate::infrastructure::database::{schema::access_tokens, DbPool};
...
impl AccessTokenRepository for PostgresAccessTokenRepository {
    ...
    async fn find_by_user_id_and_type(
        ...
        user_id: Uuid,
        token_type: &str,
    ) -> Result<Vec<AccessToken>, ApiError> {
        let mut conn = self.pool.get().map_err(|e| {
            ApiError::with_details(
                ERROR_DATABASE_ERROR,
                "Failed to get database connection",
    ...
    async fn find_valid_token_by_user_id_and_type(
        &self,
        user_id: Uuid,
        token_type: &str,
    ...
    async fn update(&self, access_token: &AccessToken) -> Result<AccessToken, ApiError> {
        ...
        let updated_token =
            diesel::update(access_tokens::table.filter(access_tokens::id.eq(access_token.id)))
                .set((
    ...
    async fn revoke_all_tokens_by_user_id_and_type(
        &self,
        user_id: Uuid,
        token_type: &str,
    ) -> Result<(), ApiError> {
    ...
    async fn delete_expired(&self) -> Result<u64, ApiError> {
        ...
        let deleted_count =
            diesel::delete(access_tokens::table.filter(access_tokens::expires_at.lt(Utc::now())))
                .execute(&mut conn)
                .map_err(|e| {
                    ApiError::with_details(
                        ERROR_DATABASE_ERROR,
