...
use crate::domain::entities::session::{Session, NewSession};
use crate::domain::repositories::session_repository::SessionRepository;
use crate::infrastructure::database::{DbPool, schema::sessions};
...
impl SessionRepository for PostgresSessionRepository {
    ...
    async fn delete_expired(&self) -> Result<u64, ApiError> {
        ...
        let deleted_count = diesel::delete(sessions::table.filter(sessions::expires_at.lt(Utc::now())))
            .execute(&mut conn)
            .map_err(|e| ApiError::with_details(
                ERROR_DATABASE_ERROR,
