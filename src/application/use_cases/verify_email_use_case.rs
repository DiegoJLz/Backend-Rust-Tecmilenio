...
use crate::shared::error_types::{ApiError, ERROR_INVALID_TOKEN, ERROR_TOKEN_ALREADY_USED, ERROR_USER_NOT_FOUND};
...
impl VerifyEmailUseCase {
    ...
    pub async fn execute(&self, token: String) -> Result<User, ApiError> {
        ...
        let verification_token = self.email_verification_repository
            .find_by_token(&token)
            .await?
        ...
        user.verify();
        let verified_user = self.user_repository
            .update(&user)
            .await?;
